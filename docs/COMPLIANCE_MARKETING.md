...
> **Hippo 话术**：
> Hippo 的 BM25 评分逻辑是公开透明的——每个相关性分数的计算公式是公开透明的。在合规审计时，你可以直接用 Python 复现每一个搜索结果的权重计算过程。不是"黑盒模型觉得它相关"，是"TF-IDF 的公式算出它相关"。这为满足《人工智能算法金融应用评价规范》（人行/市场监管总局）中关于"可解释性"的要求提供了技术基础。

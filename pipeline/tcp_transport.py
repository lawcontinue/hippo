...
class TensorReceiver:
    ...
    async def _handle_conn(self, reader: asyncio.StreamReader, _writer):
...
class TensorSender:
    ...
    用法:
        sender = TensorSender(host="169.254.100.105", port=29500)
    ...
    async def connect(self, retries: int = 3, delay: float = 1.0):
        ...
        for i in range(retries):
            try:
                ...
                print(f"[TensorSender] Connected to {self.host}:{self.port}")
                return
...
class PipelineTransport:
    ...
    用法:
        ...
        await transport.start(neighbors={"prev": None, "next": "169.254.100.105"})

...
def test_encode_decode_tensor():
    import mlx.core as mx
    from tcp_transport import decode_tensor, encode_tensor, frame_to_mlx
    ...
    assert float(mx.abs(original - recovered).max()) < 1e-5, "float32 roundtrip error"
...
def test_encode_decode_float16():
    ...
    recovered = frame_to_mlx(frame)
    mx.eval(recovered)
    assert float(mx.abs(original - recovered).max()) < 0.01, "float16 roundtrip error"
